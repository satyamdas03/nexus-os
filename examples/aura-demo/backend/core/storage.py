...
CREATE TABLE IF NOT EXISTS scenario_jobs (
  job_id TEXT PRIMARY KEY, kind TEXT, status TEXT, started_ts TEXT, done_ts TEXT,
  result_json TEXT, report_html TEXT, error TEXT
);
...
def _migrate_scenario_jobs_table(conn: sqlite3.Connection) -> None:
    """Idempotent migration: add scenario_jobs table for async stress sweeps."""
    cols = _columns(conn, "scenario_jobs")
    if not cols:
        conn.execute(
...
def init_schema(conn: sqlite3.Connection) -> None:
    ...
    _migrate_scenario_jobs_table(conn)
    conn.commit()
